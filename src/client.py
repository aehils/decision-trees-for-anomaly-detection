    ...
    try:
        ...
        while True:
            ...
            if answers['action'] == 'Set ProcessState':
                ...
                anomaly_questions = [
                    inquirer.List('variables',
                                      message="For which variable(s) do you want to raise an anomaly?",
                                      choices=list(ContinuousAnomaly),
                                      ),
                ]
                anomaly_answers = inquirer.prompt(anomaly_questions)
                print(f"Anomaly in {anomaly_answers['variables']}")
                for idx, anomaly in enumerate(ContinuousAnomaly):
                    anomaly_range_start = -25.0
                    anomaly_range_end = 25.0
                    if anomaly.value == anomaly_answers['variables']:
                        anomaly_coordinates[idx] = random.uniform(anomaly_range_start, anomaly_range_end)
                ...
                print(f"Anomaly set to {anomaly_coordinates} at {datetime.datetime.now()}")
                time.sleep(anomaly_duration)
                anomaly_coordinates = [None, None, None]
                send_anomaly_data(anomaly_coordinates=anomaly_coordinates)
                print(f"Anomaly reset to {anomaly_coordinates} at {datetime.datetime.now()}")
